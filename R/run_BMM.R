...
#' Manage isotopic ratio for tracers specified within \code{iso.prop}, \code{iso.ratio} (optional \code{iso.uncer})
#'
...
#' @param iso.ratio Character vector. Optional column names for the non-linear property ratio, such as isotopes (e.g. d13C).
#' @param iso.prop Character vector. Optional column names for the concentration of the non-linear properties respective properties (e.g. TOC for d13C).
#' @param iso.uncer Character vector. Optional column names for non-linear properties uncertainties (e.g. d13C_uncertainty).
...
#' @import progress
#' 
#' @author Thomas Chalaux-Clergue, Rémi Bizeul & Amaury Bardelle
#'
#' @export 
run.BMM <- function(data, class, mixture = "target", sample.id, tracers, uncertainty, iso.ratio, iso.prop, iso.uncer, save.dir, note, fileEncoding = "latin1",
  ...
  if (!missing(iso.ratio)) {
    dt.source[, iso.ratio] <- abs(dt.source[, iso.ratio]) # keep the absolute value of organic matter isotopes
    dt.target[, iso.ratio] <- abs(dt.target[, iso.ratio]) # keep the absolute value of organic matter isotopes
  }
  ...
  if (!missing(iso.ratio)) {
    tracers <- setdiff(tracers, iso.ratio) # remove isotopes from the list of tracers
    iso <- TRUE
  }else{
    iso <- FALSE
  }
  ...
  for (target in dt.target[[sample.id]]) {
    ...
    random.fun <- function(){
      ...
      if (isTRUE(iso)) {
        Xm.iso.cons <- Xm[iso.prop,]
        Xm.iso.ratio <- Xm[iso.ratio,]
        mnd.target.iso <- mnd.target[iso.ratio]
      }
