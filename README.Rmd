...
* [Usage](#usage)
  ...
  + [2 - Tracer Selection](#2---tracer-selection)
    ...
    - [2.4 - Discriminat Function Analysis stepwise selection](24---discriminat-function-analysis-stepwise-selection)
...
* ’Discriminant_pairs[_note].csv’, generated if `save.discrim.tests = TRUE` (default), contains the *p* value of each two-samples KS tests for each property. Set `return.tests = TRUE` to return this table as `$result.KS` (default is `FALSE`). Of note, when using the KW test, the KS test results can also help verify the KW assumption of similar distribution shape across groups.
...
Of note, it is not compulsory to provide the actual measurement uncertainty of tracers to `uncertainty` argument, although, it is hilghly recommended.
...
Finally, the `ensure.total` function ensures that the total of the predicted source contributions from all sources sum to 1 or 100 % (saves ’corrected_contrib[_note].csv’).
...
> if "Error: .onload ... 'rgags' -> it's because R version is too old need at least R.2.2
...
The upcoming version 2.3.0 update will introduce the Python version of fingR, making the sediment source fingerprinting tools available to a broader community.
...
If you encounter a clear bug, please open an issue or send an email to [Thomas Chalaux-Clergue](mailto:thomaschalaux@icloud.com).
